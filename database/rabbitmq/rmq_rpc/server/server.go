...
import (
	...
	rmqrpc "github.com/woxingliu/ha/database/rabbitmq/rmq_rpc"
)
...
func (s *Server) serveCall(d *amqp.Delivery) {
	...
	if err != nil {
		...
		log.Println("rmq_rpc server - Server - serveCall - callHandler")
	...
	if err != nil {
		log.Println("rmq_rpc server - Server - serveCall - json.Marshal")
	}
...
func (s *Server) publish(d *amqp.Delivery, body []byte, status string) {
	...
	if err != nil {
		log.Println("rmq_rpc server - Server - publish - s.conn.Channel.Publish")
	}
}
